#!/usr/bin/env python3
"""
Thin wrapper around the packaged batch runner.
"""
...
from comfy_api.batch import main
...
def expand_parameter_specs(param_specs: Dict[str, Any]) -> Dict[str, Dict]:
    ...
    Handles different parameter types: continuous, linear, values, random_seed.
    """
    expanded = {}
    ...
    for param_name, spec in param_specs.items():
        ...
        path = spec.get('path', param_name)  # Default to param name if no path
...
def generate_sobol_samples(param_specs: Dict[str, Dict], n_samples: int) -> List[Dict[str, Any]]:
    ...
    for name, spec in param_specs.items():
        ...
        # random_seed handled separately
...
def generate_grid_samples(param_specs: Dict[str, Dict]) -> List[Dict[str, Any]]:
    ...
    for name, spec in param_specs.items():
        if spec['type'] == 'continuous':
            # For continuous, create 10-point grid
...
def generate_parameter_samples(config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Dict]]:
    ...
    # Expand parameter specifications
    param_specs = expand_parameter_specs(param_configs)

    # Filter out random seeds (handled separately)
...
def set_nested_value(obj: Dict, path: str, value: Any):
    ...
    Example: "nodes.57.inputs.cfg" sets obj['57']['inputs']['cfg'] = value
    """
    parts = path.split('.')

    # Handle 'nodes.' prefix
    if parts[0] == 'nodes':
        parts = parts[1:]  # Remove 'nodes' prefix
...
def format_run_name(pattern: str, run_id: int, params: Dict[str, Any]) -> str:
    ...
    # Create flattened params for string formatting
    format_params = {'run_id': run_id}

    for key, value in params.items():
        # Clean parameter name for use in filename
...
def run_batch(config_path: str):
    ...
    # Setup metadata CSV - dynamic columns based on parameters
    ...
    print("\n💡 Next: python generate_contact_sheet.py {batch_dir}")
    print("=" * 70)
